...
- **`timestampServer` exists to cross-check `timestampGate`, not as decoration.** It's the only independent reference the server has; a per-gate `timestampServer - timestampGate` offset is the cheap way to make skew visible before it corrupts a result.
- Clock *sync* (DS3231, the planned `stage-started` sync message) reduces drift but never guarantees two gates agree — don't treat an RTC as making the checks above unnecessary.
...
**Detections are lossy by default, and a dropped one is a driver with no time.** `gate-agent` publishes at QoS 0 with a random client id and a clean session (`apps/gate-agent/src/main.ts`), and `@OnEvent` handlers swallow exceptions (`@nestjs/event-emitter` defaults `suppressErrors: true`) — so a failure anywhere from RF to DB is a log line and nothing else. `DetectionEventRecord.processed` is written but never read, so there's no retry today. Anything new on this path should assume at-least-once delivery and stay idempotent (the `startRun`/`finishRun`/`recordSplit` duplicate-ignoring in `stage-runs.service.ts` is the pattern to match).
