...
| `/stage-runs/:id/unvoid` | POST | reverses a void. 409s if a later surviving attempt would still supersede it (the message names the attempt to void first) or if it would leave two attempts open — it refuses rather than cascading a void onto a run the car actually drove |
