...
| `/stage-runs/:id/unvoid` | POST | reverses a void. Refuses outright (not forceable) if a later surviving attempt would still supersede it — the message names the attempt to void first — or if it would leave two attempts open; it never cascades a void onto a run the car actually drove. 409s with `{ displacedAttempt }`, retryable with `?force=true`, when restoring would take over as the counting attempt from a lower-numbered survivor |
