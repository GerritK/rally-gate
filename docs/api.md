...
| `/stage-runs/:id/void` | POST | strikes out an attempt (red flag). The row stays as evidence with status `VOIDED` but stops counting, and the vehicle is freed so the **start gate opens the re-run itself** on its next pass — both ends stay gate-timed, no restart time is typed in. No un-void; see "Voiding" in `event-model.md` |
